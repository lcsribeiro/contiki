...
#include "usci.h" // for CC2650 Pin configuration, temp
#include "util.h"
//#include "ioc.h"
//lucas
...
void IINCHIP_CSoff(void)
{
  ...
  //wizspi_cs(WIZ_SPI1, VAL_LOW);
	...
	GPIO_writeDio(WIZ_SCS_PIN,0); //WIZ_SCS_PORT &= ~WIZ_SCS_PIN;
}
/*
  Habilita a fonte do clock
*/
void IINCHIP_CSon(void)
{
  ...
  //wizspi_cs(WIZ_SPI1, VAL_HIGH);
	...
	GPIO_writeDio(WIZ_SCS_PIN,1); //WIZ_SCS_PORT |= WIZ_SCS_PIN;
}
...
void IINCHIP_READ_COMMON_SEQB(uint16 addr, uint8 len, uint8 * data, uint8 byte_en )
{
  ...
  {
    cntl_bits = (uint8)(CB_TAIL_COMMONREG_RD_SEQ | byte_en);  
    if( !(len == 1 || len == 2 || len == 4 ) )
    {
      len = 0;
    }
    PRINTF("cntl_bits : %X, len: %d \r\n", cntl_bits, byte_en);
  }
...
{  
  ...
  {
    cntl_bits = (uint8)(CB_TAIL_COMMONREG_WR_SEQ | byte_en);     
    if( !(len == 1 || len == 2 || len == 4 ) )
    {
      len = 0;
    }
